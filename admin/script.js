...
async function forzarSync() {
  ...
  try {
    const [t, i] = await Promise.all([apiGet('turnos'), apiGet('inventario')]);
...
function renderInicio() {
  ...
  const hoyN    = turnos.filter(t => t.fecha === hoy).length;
  const mesN    = turnos.filter(t => (t.fecha||'').startsWith(mes)).length;

  // Stats
  document.getElementById('stPend').textContent = pend;
  document.getElementById('stConf').textContent = conf;
  document.getElementById('stHoy').textContent  = hoyN;
  document.getElementById('stMes').textContent  = mesN;
  ...
  if (!hoyTurnos.length) {
    listEl.innerHTML = '<p class="today-empty">Sin turnos para hoy</p>';
  } else {
    listEl.innerHTML = hoyTurnos.map(t => `
      <li class="today-item">
  ...
  if (!pendTurnos.length) {
    pendList.innerHTML = '<p class="today-empty">Sin turnos pendientes</p>';
  } else {
    pendList.innerHTML = pendTurnos.map(t => {
  ...
  tbody.innerHTML = sorted.map(t => {
    ...
    return `<tr>
      ...
      <td class="td-actions">
        ...
        ` : ''}
        ${t.estado==='confirmado' && waR ? `
          <a href="${waR}" target="_blank" class="btn btn-sm" style="background:var(--bg-yellow);border:none;color:var(--text);padding:.28rem .7rem">⏰ WA</a>
        ` : ''}
...
function renderDias() {
  document.getElementById('diasChips').innerHTML = DIAS_N.map((d,i) => `
    <div class="chip ${agTemp.diasHabilitados.includes(i) ? 'on' : ''}"
         onclick="toggleDia(${i})">${d}</div>
  `).join('');
}

function toggleDia(i) {
  const arr = agTemp.diasHabilitados;
...
function guardarDias() {
  const ag = getAgenda();
  ag.diasHabilitados = agTemp.diasHabilitados;
...
function renderSlots() {
  document.getElementById('slotsChips').innerHTML = agTemp.slots.map(s => `
    <div class="chip on-gold" style="display:flex;align-items:center;gap:.3rem">
      ${s}
      <span onclick="quitarSlot('${s}')" style="cursor:pointer;color:var(--text-muted);font-size:.9rem;line-height:1">×</span>
    </div>
  `).join('');
}
...
function agregarSlot() {
  ...
  agTemp.slots.push(v); agTemp.slots.sort();
  renderSlots();
}

function quitarSlot(s) {
  agTemp.slots = agTemp.slots.filter(x => x !== s);
  renderSlots();
}
...
function guardarDuraciones() {
  ...
  svcs.forEach(s => {
    const el = document.getElementById('dur_' + s.id);
    if (el) s.duracion = parseInt(el.value) || s.duracion;
  });
  ag.servicios = svcs;
  DB.set('agenda_config', ag);
...
function bloquearFecha() {
  const v = document.getElementById('fechaBloqueo').value;
  if (!v) return;
  const ag = getAgenda();
  if (!ag.diasBloqueados.includes(v)) ag.diasBloqueados.push(v);
  DB.set('agenda_config', ag);
  agTemp = JSON.parse(JSON.stringify(ag));
  renderBloqueadas();
...
function desbloquearFecha(v) {
  const ag = getAgenda();
  ag.diasBloqueados = ag.diasBloqueados.filter(d => d !== v);
  DB.set('agenda_config', ag);
  agTemp = JSON.parse(JSON.stringify(ag));
  renderBloqueadas();
}
...
function renderBloqueadas() {
  const ag  = getAgenda();
  const div = document.getElementById('fechasBloqueadas');
  div.innerHTML = ag.diasBloqueados.length
    ? ag.diasBloqueados.sort().map(d => `
        <div class="chip danger" style="display:flex;align-items:center;gap:.3rem">
          ${d}
...
function renderInvTabs() {
  ...
  document.getElementById('invTabs').innerHTML = cats.map(c => `
    <div class="chip ${(invCatActiva === c || (!invCatActiva && c==='Todos')) ? 'on-gold' : ''} scrollable"
         onclick="setInvCat('${c}')">${c}</div>
  `).join('');
}
...
function renderInventario() {
  ...
  tbody.innerHTML = lista.map(p => {
    ...
    const fotoEl = (fotoSrc && fotoSrc !== '⏳')
      ? `<img src="${fotoSrc}" style="width:38px;height:38px;border-radius:6px;object-fit:cover;border:1px solid var(--border)" onerror="this.style.display='none'">`
...
function exportCSV() {
  ...
  const rows = getInventario().map(p => [
    p.nombre||'', p.marca||'', p.categoria||'', p.color||'',
    p.stock||0,
    Number(p.stock)<=0?'Sin stock':Number(p.stock)<=2?'Stock bajo':'Ok',
    p.visibilidad||'privado', p.notas||'',
  ]);
  const csv = [h,...rows].map(r => r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
  dlFile(new Blob([csv],{type:'text/csv;charset=utf-8;'}), 'inventario.csv');
...
function setVis(val) {
  ...
  if (!bp || !bpu) return;
  const activeStyle = 'border:1.5px solid var(--accent);background:rgba(232,144,144,.1);color:var(--text);font-weight:500';
  const inactiveStyle = 'border:1.5px solid var(--border);background:var(--bg);color:var(--text-muted);font-weight:400';
  bp.style.cssText  = val === 'privado' ? activeStyle : inactiveStyle;
  bpu.style.cssText = val === 'publico' ? activeStyle : inactiveStyle;
}
...
function previewFoto(e) {
  ...
  reader.onload = ev => {
    ...
    img.onload = () => {
      ...
      window._fotoB64 = b64;
      document.getElementById('mFotoPreview').src = b64;
      document.getElementById('mFotoPreview').style.display = 'block';
      document.getElementById('fotoLabel').textContent = '📷 Cambiar foto';
    };
...
function guardarProducto() {
  ...
  if (window._fotoB64 && window._fotoB64.startsWith('data:')) {
    ...
    apiPost({ action: 'uploadFoto', id: producto.id, b64: window._fotoB64, nombre: producto.id })
      .then(() => showToast('✓ Guardado en Drive'));
  } else {
...
function renderCategorias() {
  ...
  document.getElementById('catLista').innerHTML = cats.length
    ...
    : '<p style="color:var(--text-muted);font-size:.84rem">Sin categorías. Agregá la primera.</p>';
}

function agregarCategoria() {
  const v = document.getElementById('nuevaCat').value.trim();
  if (!v) return;
  const cats = getCategorias();
...
function editarCat(i) {
  ...
  mostrarInput({
    ...
    onOk: (nuevo) => {
      ...
      const inv = getInventario().map(p => p.categoria===cats[i] ? {...p,categoria:nuevo} : p);
      saveInventario(inv);
      cats[i] = nuevo;
      saveCategorias(cats);
      renderCategorias();
...
function confirmarInput() {
  const v = document.getElementById('inpVal').value.trim();
  if (!v) return;
  cerrarModal('modalInput');
  _inputOk?.(v);
}
...
// Override doLogout para path correcto desde admin/
function doLogout() {
